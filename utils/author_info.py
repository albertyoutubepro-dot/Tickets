...
class TicketAuthorInfoSystem:
    ...
    async def get_user_info(self, guild, user_id, fetch_from_api=True):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ Failed to chunk guild {guild.id}: {chunk_error}")
            ...
            if fetch_from_api:
                try:
                    ...
                    logger.warning(f"⚠️ HTTP error fetching user {user_id}: {e}")
    ...
    def _create_left_user_embed(self, info):
        ...
        embed.add_field(
            name="Status",
            value="```⚠️ User has left the server```",
            inline=False
        )
...
class UserAvatarView(discord.ui.View):
    ...
    @discord.ui.button(label="View Avatar", style=discord.ButtonStyle.primary, emoji="❤️")
