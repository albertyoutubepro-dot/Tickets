...
class SupportSystem(commands.Cog):
            ...
            async with self.bot.db.cursor() as cur:
                await cur.execute("""
                    CREATE TABLE IF NOT EXISTS tickets (
                        ...
                        embed_footer TEXT DEFAULT 'Support System',
                ...
                try:
                    await cur.execute("ALTER TABLE tickets ADD COLUMN embed_footer TEXT DEFAULT 'Support System'")
                except:
    ...
    async def reset_categories(self, ctx: commands.Context):
        ...
        try:
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="🗑️ Categories Reset" if success else "❌ Error",
                description=message,
                color=0x00D4FF if success else 0xFF6B6B,
                timestamp=current_time
            )
                ...
                if current_claimer_id:
                    ...
                    embed = discord.Embed(
                        ...
                        description=f"**This ticket is already being handled by another agent.**\n\n"
                                   ...
                                   f"🏷️ **Agent Name:** {claimer.display_name if claimer else 'Unknown Agent'}\n"
    ...
    async def faq(self, ctx: commands.Context):
        ...
        try:
            ...
            embed.set_footer(text="Support System • Interactive FAQ")
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=f"**Maintenance mode has been {status.lower()}.**\n\n"
                           f"{'⚠️ **Ticket creation is now disabled.** Users cannot create new tickets.' if new_mode else '✅ **Ticket creation is now enabled.** Users can create tickets normally.'}\n\n"
...
class FAQCategoryView(discord.ui.View):
    ...
    async def faq_category_select(self, interaction: discord.Interaction, select: discord.ui.Select):
        try:
            ...
            embed.set_footer(text=f"Support System • {category.replace('_', ' ').title()} FAQ")
